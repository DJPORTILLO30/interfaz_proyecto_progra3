...
class _UserCardListState extends State<UserCardList> {
  Future<List> getData() async {
    final response = await http.get(Uri.parse("http://192.168.1.7:8080/cliente"));
                ...
                children: <Widget>[
                  Button(
                    ...
                    onPressed: (){
                      Navigator.push(
                          context,
                          MaterialPageRoute(builder: (BuildContext context) => UserFound(Id: _controllerSearch.text,)));
                    },
                  ),
                  Button(
                    text: "Nuevo usuario",
  ...
  Widget build(BuildContext context) {
    return new ListView.builder(
        ...
        itemBuilder: (context,i) {
          return UserCard(id: list[i]['id'].toString(),nombre: list[i]['nombre'].toString(),email: list[i]['email'].toString(),edad: list[i]['edad'].toString(),sexo: list[i]['sexo'].toString(),pais: list[i]['pais'].toString(),password: list[i]['password'].toString(),fecha: list[i]['fecha'].toString(),rol: list[i]['rol'].toString());
        }
    );
  }
